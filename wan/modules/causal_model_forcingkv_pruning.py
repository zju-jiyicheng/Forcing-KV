...
def _fill_cache_from_token_indices_(cache_tensor, candidate_tensor, keep_token_indices, cache_name):
    keep_token_indices = keep_token_indices.to(device=candidate_tensor.device, dtype=torch.long)
    keep_tokens = keep_token_indices.numel()
    if keep_tokens > cache_tensor.shape[1]:
        raise ValueError(f"{cache_name} capacity is {cache_tensor.shape[1]}, got {keep_tokens} selected tokens")
    if keep_tokens == 0:
        return 0

    selected_tokens = candidate_tensor.index_select(dim=1, index=keep_token_indices).contiguous()
    cache_tensor[:, :keep_tokens].copy_(selected_tokens)
    if keep_tokens < cache_tensor.shape[1]:
        cache_tensor[:, keep_tokens:].zero_()
    return keep_tokens
...
class CausalWanSelfAttention(nn.Module):
    ...
    shared_dynamic_token_indices = None
    ...
    def _build_dynamic_candidate_tokens(
        self,
        old_temporal_k,
        old_temporal_v,
        new_temporal_k,
        ...
        frame_tokens = grid_h * grid_w
        if old_temporal_k.shape[1] < frame_tokens or new_temporal_k.shape[1] < 3 * frame_tokens:
            CausalWanSelfAttention.shared_dynamic_token_indices = None
            return None, None

        temporal_keep_frames = old_temporal_k.shape[1] // frame_tokens
        if temporal_keep_frames < 1:
            raise ValueError(f"Random/uniform temporal mode requires at least one cached temporal frame, got {temporal_keep_frames}")
        ...
        candidate_frames_k = torch.cat([old_frames_k[:, -1:], new_frames_k], dim=1).contiguous()
        candidate_frames_v = torch.cat([old_frames_v[:, -1:], new_frames_v], dim=1).contiguous()
        candidate_tokens_k = candidate_frames_k.reshape(
            candidate_frames_k.shape[0],
            4 * frame_tokens,
            candidate_frames_k.shape[3],
            candidate_frames_k.shape[4],
        ).contiguous()
        candidate_tokens_v = candidate_frames_v.reshape(
            candidate_frames_v.shape[0],
            4 * frame_tokens,
            candidate_frames_v.shape[3],
            candidate_frames_v.shape[4],
        ).contiguous()
        return candidate_tokens_k, candidate_tokens_v
    ...
    def random_compression(
        ...
        candidate_tokens_k, candidate_tokens_v = self._build_dynamic_candidate_tokens(
            old_temporal_k, old_temporal_v, new_temporal_k, new_temporal_v, grid_h, grid_w
        )
        if candidate_tokens_k is None:
            return 0

        total_candidate_tokens = candidate_tokens_k.shape[1]
        keep_token_count = int(round(total_candidate_tokens * max(0.0, min(1.0, float(budget)))))
        if keep_token_count > total_candidate_tokens:
            raise ValueError(f"Requested {keep_token_count} dynamic tokens, but only {total_candidate_tokens} candidate tokens exist")

        if layer_idx == 1:
            if keep_token_count > 0:
                keep_indices = torch.randperm(total_candidate_tokens, device=candidate_tokens_k.device)[:keep_token_count]
                keep_indices = torch.sort(keep_indices).values
            else:
                keep_indices = torch.empty((0,), device=candidate_tokens_k.device, dtype=torch.long)
            CausalWanSelfAttention.shared_dynamic_token_indices = keep_indices.detach()
        else:
            keep_indices = CausalWanSelfAttention.shared_dynamic_token_indices
            if keep_indices is None:
                return 0

        valid_tokens = _fill_cache_from_token_indices_(
            dynamic_cache_k,
            candidate_tokens_k,
            keep_indices,
            "group_dynamic_temporal_k",
        )
        _fill_cache_from_token_indices_(
            dynamic_cache_v,
            candidate_tokens_v,
            keep_indices,
            "group_dynamic_temporal_v",
        ...
        if layer_idx == 1:
            if keep_token_count > 0:
                keep_indices = torch.linspace(
                    0,
                    total_candidate_tokens - 1,
                    steps=keep_token_count,
                    device=candidate_tokens_k.device,
                ).round().long()
            else:
                keep_indices = torch.empty((0,), device=candidate_tokens_k.device, dtype=torch.long)
            CausalWanSelfAttention.shared_dynamic_token_indices = keep_indices.detach()
    ...
    def forward(
        ...
        dynamic_context_len_hw = max(0.0, float(getattr(self.args, "dynamic_context_length", 3)))
        ...
        dynamic_keep_tokens = int(round(int(HW) * dynamic_context_len_hw))
        ...
        if not use_grouped_kv:
            ...
            pruning_method = str(getattr(self.args, "pruning_method", "dynamic")).lower()
            if pruning_method in {"random", "uniform"}:
                k2 = torch.cat([kv_cache["group_sink_temporal_k"], dynamic_k, cur_temporal_k], dim=1).contiguous()
                v2 = torch.cat([kv_cache["group_sink_temporal_v"], dynamic_v, cur_temporal_v], dim=1).contiguous()
            else:
            ...
            if update_kv_cache:
                ...
                if not self.args.dynamic_temporal_enabled:
                    ...
                    pruning_budget = float(getattr(self.args, "budget", getattr(self.args, "sim_retention_ratio", 0.5)))
                    if pruning_method == "dynamic":
                        ...
                        dynamic_valid_tokens = self.random_compression(
                            layer_idx,
                            old_temporal_k,
                            old_temporal_v,
                            new_temporal_k,
                        ...
                        dynamic_valid_tokens = self.uniform_compression(
                            layer_idx,
                            old_temporal_k,
                            old_temporal_v,
                            new_temporal_k,
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _apply_offline_head_allocation(self, kv_cache):
        ...
        dynamic_keep_frames = max(0.0, float(getattr(self.args, "dynamic_context_length", 0)))
        ...
        for layer_idx, cur_cache in enumerate(kv_cache):
            ...
            # Dynamic Cache
            dynamic_keep = int(round(dynamic_keep_frames * frame_tokens))
