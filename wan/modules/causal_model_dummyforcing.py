...
class CausalWanSelfAttention(nn.Module):
    ...
    #     def qkv_fn(inp):
    #         q = self.norm_q(self.q(inp)).view(b, s, n, d)
    #         k = self.norm_k(self.k(inp)).view(b, s, n, d)
    #         v = self.v(inp).view(b, s, n, d)
    #         return q, k, v

    #     def groups_ready():
    #         req = ("headgroup_first", "headgroup_mid", "headgroup_last")
    #         if any(k not in kv_cache for k in req):
    #             return False
    #         hf = list(kv_cache["headgroup_first"])
    #         hm = list(kv_cache["headgroup_mid"])
    #         hl = list(kv_cache["headgroup_last"])
    #         all_heads = hf + hm + hl
    #         if len(all_heads) != self.num_heads:
    #             return False
    #         if len(set(all_heads)) != self.num_heads:
    #             return False
    #         return True
    ...
    #         hl = list(kv_cache["headgroup_last"])
    #         sink_heads = hf + hl
    #         return hf, hm, hl, sink_heads

    #     def update_full_cache():
    #         full_k_all = torch.cat([kv_cache["full_sink_k"], kv_cache["full_local_k"], roped_key], dim=1)
    #         full_v_all = torch.cat([kv_cache["full_sink_v"], kv_cache["full_local_v"], v], dim=1)

    #         kv_cache["full_sink_k"] = full_k_all[:, :HW].contiguous()
    #         kv_cache["full_sink_v"] = full_v_all[:, :HW].contiguous()

    #         kv_cache["full_local_k"] = (
    #             full_k_all[:, -3 * HW * mid_context_len:] if cur_AR_step > mid_context_len - 1 else full_k_all[:, HW:]
    #         ).contiguous()
    #         kv_cache["full_local_v"] = (
    #             full_v_all[:, -3 * HW * mid_context_len:] if cur_AR_step > mid_context_len - 1 else full_v_all[:, HW:]
    #         ).contiguous()

    #     def refresh_sparse_from_full():
    #         # group 未就绪时，不要把 head 维切成 0，直接保持 full 形态
    #         if not groups_ready():
    #             kv_cache["sink_k"] = kv_cache["full_sink_k"].contiguous()
    #             kv_cache["sink_v"] = kv_cache["full_sink_v"].contiguous()
    #             kv_cache["local_k"] = kv_cache["full_local_k"].contiguous()
    #             kv_cache["local_v"] = kv_cache["full_local_v"].contiguous()
    #             return

    #         _, mid_heads, _, sink_heads = get_groups()
    #         kv_cache["sink_k"] = kv_cache["full_sink_k"][:, :, sink_heads, :].contiguous()
    #         kv_cache["sink_v"] = kv_cache["full_sink_v"][:, :, sink_heads, :].contiguous()
    #         kv_cache["local_k"] = kv_cache["full_local_k"][:, :, mid_heads, :].contiguous()
    #         kv_cache["local_v"] = kv_cache["full_local_v"][:, :, mid_heads, :].contiguous()
    ...
    #         and full_every > 0
    #         and ((cur_AR_step - framecache_step) % full_every == 0)
    #     )
    #     do_full = (cur_AR_step <= framecache_step) or is_recache or do_periodic_full

    #     k0 = None

    #     if do_full:
    #         k0 = torch.cat([kv_cache["full_sink_k"], kv_cache["full_local_k"], roped_key], dim=1)
    #         v0 = torch.cat([kv_cache["full_sink_v"], kv_cache["full_local_v"], v], dim=1)
    #         x = attention(roped_query, k0, v0)

    #         if update_kv_cache:
    #             update_full_cache()
    #             # 只在分组可用后同步 sparse cache
    #             if groups_ready():
    #                 refresh_sparse_from_full()

    #     else:
    #         # 理论上切到 sparse 时 groups 应该已准备好；否则安全回退 full
    #         if not groups_ready():
    #             k0 = torch.cat([kv_cache["full_sink_k"], kv_cache["full_local_k"], roped_key], dim=1)
    #             v0 = torch.cat([kv_cache["full_sink_v"], kv_cache["full_local_v"], v], dim=1)
    #             x = attention(roped_query, k0, v0)
    ...
    #             # 若头维不匹配，先从 full cache 重建 sparse cache
    #             if kv_cache["sink_k"].shape[2] != len(sink_heads) or kv_cache["sink_v"].shape[2] != len(sink_heads):
    #                 refresh_sparse_from_full()
    #             if kv_cache["local_k"].shape[2] != len(mid_heads) or kv_cache["local_v"].shape[2] != len(mid_heads):
    #                 refresh_sparse_from_full()

    #             q1, k1, v1, q2, k2, v2 = extract_heads_triton(
    #                 roped_query, roped_key, v, mid_heads, sink_heads
    #             )

    #             k1_all = torch.cat([kv_cache["sink_k"], k1], dim=1).contiguous()
    #             v1_all = torch.cat([kv_cache["sink_v"], v1], dim=1).contiguous()
    #             k2_all = torch.cat([kv_cache["local_k"], k2], dim=1).contiguous()
    #             v2_all = torch.cat([kv_cache["local_v"], v2], dim=1).contiguous()

    #             x1 = attention(q1, k1_all, v1_all)
    #             x2 = attention(q2, k2_all, v2_all)
    ...
    #             if update_kv_cache:
    #                 kv_cache["sink_k"] = k1_all[:, -HW:, :].contiguous()
    #                 kv_cache["sink_v"] = v1_all[:, -HW:, :].contiguous()
    #                 kv_cache["local_k"] = k2_all[:, -3 * HW * mid_context_len:, :].contiguous()
    #                 kv_cache["local_v"] = v2_all[:, -3 * HW * mid_context_len:, :].contiguous()

    #                 # 周期全量依赖 full_*，所以每步都维护
    #                 update_full_cache()

    #     if cur_AR_step == framecache_step and update_kv_cache:
    #         # 该 step 一定走 full 或回退 full，k0 可用
    #         self.frame_attn_score = online_head_classification(roped_query, k0, self.args.ar_start)
    #         kv_cache["frame_attn_score"] = self.frame_attn_score
