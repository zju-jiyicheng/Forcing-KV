...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        spatial_context_len_hw_raw = getattr(self.args, "spatial_context_length", 1)
        spatial_context_len_hw = int(spatial_context_len_hw_raw)

        sink_tokens = max(0, int(self.sink_size)) * int(HW)
        temporal_keep_tokens = max(0, int(HW) * temporal_context_len_hw)
        spatial_keep_tokens = max(0, int(HW) * spatial_context_len_hw)
        ...
        if use_full_kv:
            ...
            q1, k1, v1, q2, k2, v2 = extract_heads_triton(
                roped_query, roped_key, v, headgroup_mid, headgroup_last
            )
            k1 = torch.cat([kv_cache["group_sink_spatial_k"], kv_cache["group_spatial_k"], k1], dim=1).contiguous()
            ...
            x[:, :, headgroup_mid, :] = x2
            if update_kv_cache:
                full_local_k_all = torch.cat([kv_cache["local_k"], roped_key], dim=1).contiguous()
                full_local_v_all = torch.cat([kv_cache["local_v"], v], dim=1).contiguous()
                if history_keep_tokens > 0:
                    kv_cache['local_k'] = full_local_k_all[:, -history_keep_tokens:].contiguous()
                    kv_cache['local_v'] = full_local_v_all[:, -history_keep_tokens:].contiguous()
                else:
                    kv_cache['local_k'] = full_local_k_all[:, :0].contiguous()
                    kv_cache['local_v'] = full_local_v_all[:, :0].contiguous()

                grouped_spatial_k_all = torch.cat([kv_cache["group_spatial_k"], k1[:, -s:].contiguous()], dim=1).contiguous()
                grouped_spatial_v_all = torch.cat([kv_cache["group_spatial_v"], v1[:, -s:].contiguous()], dim=1).contiguous()
                if spatial_keep_tokens > 0:
                    kv_cache['group_spatial_k'] = grouped_spatial_k_all[:, -spatial_keep_tokens:].contiguous()
                    kv_cache['group_spatial_v'] = grouped_spatial_v_all[:, -spatial_keep_tokens:].contiguous()
                else:
                    kv_cache['group_spatial_k'] = grouped_spatial_k_all[:, :0].contiguous()
                    kv_cache['group_spatial_v'] = grouped_spatial_v_all[:, :0].contiguous()

                temporal_history = torch.cat([kv_cache["group_temporal_k"], k2[:, -s:].contiguous()], dim=1).contiguous()
                temporal_history_v = torch.cat([kv_cache["group_temporal_v"], v2[:, -s:].contiguous()], dim=1).contiguous()
                if temporal_keep_tokens > 0:
                    kv_cache['group_temporal_k'] = temporal_history[:, -temporal_keep_tokens:].contiguous()
                    kv_cache['group_temporal_v'] = temporal_history_v[:, -temporal_keep_tokens:].contiguous()
                else:
                    kv_cache['group_temporal_k'] = temporal_history[:, :0].contiguous()
                    kv_cache['group_temporal_v'] = temporal_history_v[:, :0].contiguous()
                kv_cache['frame_tokens'] = int(HW)
        x = x.flatten(2)
        x = self.o(x)
        return x
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        self._switch_context_printed = False
    ...
    def _load_offline_head_groups(self):
        ...
        for default_idx, item in layer_items:
            ...
            spatial = sorted({int(h) for h in item.get("local_head", item.get("local_heads", [])) if 0 <= int(h) < self.num_heads})
            temporal = sorted({int(h) for h in item.get("temporal_head", item.get("temporal_heads", [])) if 0 <= int(h) < self.num_heads})
            if sorted(spatial + temporal) != list(range(self.num_heads)):
                raise ValueError(
                    f"Offline head groups must cover all heads exactly once at layer {layer_idx}, got {sorted(spatial + temporal)}"
                )
            groups[layer_idx] = {"local_head": spatial, "temporal_head": temporal}

        for layer_idx in range(self.num_layers):
            groups.setdefault(layer_idx, {"local_head": [], "temporal_head": list(range(self.num_heads))})
    ...
    def _apply_offline_head_allocation(self, kv_cache):
        if len(kv_cache) == 0:
            return
        if "headgroup_last" in kv_cache[1] and "headgroup_mid" in kv_cache[1]:
            return
        ...
        for layer_idx, cur_cache in enumerate(kv_cache):
            ...
            spatial_heads = list(layer_group["local_head"])
            ...
            spatial_keep_tokens = max(0, int(getattr(self.args, "spatial_context_length", 1))) * frame_tokens
            temporal_keep_tokens = max(0, int(getattr(self.args, "temporal_context_length", 3))) * frame_tokens

            full_sink_k = cur_cache["sink_k"]
            full_sink_v = cur_cache["sink_v"]
            full_local_k = cur_cache["local_k"]
            full_local_v = cur_cache["local_v"]
            sink_tokens = full_sink_k.shape[1]

            spatial_sink_k = full_sink_k[:, :, spatial_heads, :].contiguous().clone()
            spatial_sink_v = full_sink_v[:, :, spatial_heads, :].contiguous().clone()
            if spatial_keep_tokens > 0:
                spatial_recent_k = full_local_k[:, -spatial_keep_tokens:, spatial_heads, :].contiguous().clone()
                spatial_recent_v = full_local_v[:, -spatial_keep_tokens:, spatial_heads, :].contiguous().clone()
            else:
                spatial_recent_k = full_local_k[:, :0, spatial_heads, :].contiguous().clone()
                spatial_recent_v = full_local_v[:, :0, spatial_heads, :].contiguous().clone()
            cur_cache["group_sink_spatial_k"] = spatial_sink_k
            cur_cache["group_sink_spatial_v"] = spatial_sink_v
            cur_cache["group_spatial_k"] = spatial_recent_k
            cur_cache["group_spatial_v"] = spatial_recent_v
            ...
            if temporal_keep_tokens > 0:
                cur_cache["group_temporal_k"] = full_local_k[:, -temporal_keep_tokens:, temporal_heads, :].contiguous().clone()
                cur_cache["group_temporal_v"] = full_local_v[:, -temporal_keep_tokens:, temporal_heads, :].contiguous().clone()
            else:
            ...
            if (
                not self._switch_context_printed
                and (not dist.is_initialized() or dist.get_rank() == 0)
            ):
                current_tokens = int(self.num_frame_per_block) * frame_tokens
                print(
                    "[ForcingKV switch] "
                    # f"spatial_heads={len(spatial_heads)}, "
                    # f"temporal_heads={len(temporal_heads)} | "
                    f"spatial: sink={sink_tokens}, context={spatial_keep_tokens}, current={current_tokens}, total={sink_tokens + spatial_keep_tokens + current_tokens} | "
                    f"temporal: sink={sink_tokens}, context={temporal_keep_tokens}, current={current_tokens}, total={sink_tokens + temporal_keep_tokens + current_tokens}"
                )
                self._switch_context_printed = True
    ...
    def _forward_inference(
        ...
        if t[0,0] == 0 and AR_step == self.args.ar_start and not is_recache:
            # Switch once at the first denoising step of ar_start using offline
            # spatial/temporal head groups.
