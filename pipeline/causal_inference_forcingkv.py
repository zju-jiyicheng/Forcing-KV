...
class CausalInferencePipeline_ForcingKV(torch.nn.Module):
    def __init__(
            self,
            args,
            device,
        ...
        self.generator = WanDiffusionWrapper(
            **getattr(args, "model_kwargs", {}), is_causal=True, method='forcingkv') if generator is None else generator
        # update dummy forcing args
        self.generator.model.args = args.forcingkv
        for blk in self.generator.model.blocks:
            blk.self_attn.args = args.forcingkv
    ...
    def _initialize_kv_cache(self, batch_size, dtype, device, kv_cache_size_override: int | None = None):
        ...
        for _ in range(self.num_transformer_blocks):
            kv_cache1.append({
                ...
                # # MODIFIED
                # "full_sink_k": torch.zeros([batch_size, 0,  12, 128], dtype=dtype, device=device),
                # "full_sink_v": torch.zeros([batch_size, 0,  12, 128], dtype=dtype, device=device),
                # "full_local_k": torch.zeros([batch_size, 0, 12, 128], dtype=dtype, device=device),
                # "full_local_v": torch.zeros([batch_size, 0, 12, 128], dtype=dtype, device=device),
            })
