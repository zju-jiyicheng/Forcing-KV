...
class CausalInferencePipeline_Realtime(torch.nn.Module):
    def __init__(
            self,
            args,
            device,
        ...
        if hasattr(args, "model_name"):
            model_kwargs.setdefault("model_name", args.model_name)
        model_kwargs["method"] = "forcingkv_realtime" if getattr(args, "method", "") == "forcingkv_realtime" else "realtime"
        self.generator = WanDiffusionWrapper(
            **model_kwargs, is_causal=True) if generator is None else generator
        if getattr(args, "method", "") == "forcingkv_realtime":
