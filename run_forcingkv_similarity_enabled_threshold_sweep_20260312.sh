...
BKP="$CFG.bak_similarity_enabled_threshold_sweep_20260312"
LOG_DIR="$ROOT/experiment_logs/forcingkv_similarity_enabled_threshold_sweep_20260312"
...
for th in "${thresholds[@]}"; do
  th_tag="${th/./p}"
  out_folder="videos/inference/forcingkv_longlive_similarity_enabled/th${th_tag}"
  log_file="$LOG_DIR/run_th${th_tag}.log"

  sed -i -E "s/^  kv_merge_enabled:.*/  kv_merge_enabled: false/" "$CFG"
  sed -i -E "s/^  similarity_prune_enabled:.*/  similarity_prune_enabled: true/" "$CFG"
  sed -i -E "s/^  similarity_threshold:.*/  similarity_threshold: ${th}/" "$CFG"
  sed -i -E "s#^data_path:.*#data_path: prompts/example_prompts.txt#" "$CFG"
  sed -i -E "s#^output_folder:.*#output_folder: ${out_folder}#" "$CFG"

  echo "===== START similarity_prune_enabled=true, similarity_threshold=${th}, kv_merge_enabled=false =====" | tee "$log_file"
  set +e
  ...
  run_rc=${PIPESTATUS[0]}
  set -e
  echo "===== END similarity_prune_enabled=true, similarity_threshold=${th}, rc=${run_rc} =====" | tee -a "$log_file"
  ...
  echo "true,${th},false,prompts/example_prompts.txt,${out_folder},${fps_count},${fps_avg},${status},${log_file}" | tee -a "$SUMMARY"
done
