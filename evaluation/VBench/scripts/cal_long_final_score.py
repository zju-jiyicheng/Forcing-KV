...
def get_quality_score(normalized_score):
    quality_score = []
    #MODIFIED HERE TO LONG VERSION
    for key in QUALITY_LIST_LONG:
        quality_score.append(normalized_score[key])
    quality_score = sum(quality_score)/sum([DIM_WEIGHT[i] for i in QUALITY_LIST_LONG])
...
if __name__=="__main__":
    ...
    print(f"your submission info: \n{upload_dict} \n")
    print('+------------------|------------------+')
    print(f'|     total & quality score|{quality_score}|')
    # print(f'|    semantic score|{semantic_score}|')
    # print(f'|       total score|{final_score}|')
    print('+------------------|------------------+')

    # ✅ 保存到 output_path/result.json
    ...
    result_obj = {
        ...
        "submission_info": upload_dict,
        "total & quality_score": float(quality_score),
